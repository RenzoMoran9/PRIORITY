...
(function () {
  ...
  async function enviarADrive(nombre, contenido) {
    ...
    try {
      const r = await fetch(drive.url, {
        method: "POST",
        // texto plano: evita la comprobación previa del navegador (CORS)
        headers: { "Content-Type": "text/plain;charset=utf-8" },
        body: JSON.stringify({ clave: drive.clave, nombre: nombre, contenido: contenido }),
        redirect: "follow",
      });
      const txt = await r.text();
      try { return JSON.parse(txt); } catch (e) { return { ok: r.ok }; }
    } catch (e) {
  ...
  function abrirConfigDrive(btn) {
    ...
    pop.querySelector("#drive-probar").addEventListener("click", async () => {
      ...
      if (res && res.ok) {
        ...
        drive.url = previa;
        est.textContent = "No respondió. Revisa que el acceso sea «Cualquier usuario» y que la clave del código coincida.";
