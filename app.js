...
(function () {
  ...
  let mdPendiente = null; // vínculo recordado que solo espera confirmar el permiso
  ...
  // El vínculo se recuerda entre sesiones: el navegador permite guardar la
  // referencia al archivo (no su contenido) para no volver a elegirlo cada vez.
  function idbAbrir() {
    return new Promise((res, rej) => {
      const r = indexedDB.open("priority_fs", 1);
  ...
  async function idbLeer(clave) {
    ...
    return new Promise((res, rej) => {
      ...
      const q = tx.objectStore("handles").get(clave);
      q.onsuccess = () => res(q.result || null);
      q.onerror = () => rej(q.error);
    });
  }

  // Al abrir la app: si ya había un archivo vinculado, se reanuda solo (o queda
  // a un clic, si el navegador exige confirmar el permiso de nuevo).
  async function restaurarVinculoResumen() {
    if (!window.indexedDB) return;
    try {
      const h = await idbLeer("resumen");
  ...
  async function vincularResumen() {
    ...
    // Reconexión de un vínculo anterior: basta con dar permiso, sin volver a elegir archivo
    if (mdPendiente) {
      try {
        const ok = await mdPendiente.requestPermission({ mode: "readwrite" });
        if (ok === "granted") {
          mdHandle = mdPendiente; mdPendiente = null;
          actualizarBotonResumenVinculo();
          await escribirResumen(true);
          return;
        }
      } catch (e) {}
      mdPendiente = null;   // si no se pudo, se elige archivo de nuevo
    }
    try {
      ...
      try { await idbGuardar("resumen", mdHandle); } catch (e) {}
      await escribirResumen(true);
      actualizarBotonResumenVinculo();
    } catch (e) {
      if (e && e.name === "AbortError") return;
  ...
  function actualizarBotonResumenVinculo() {
    ...
    if (mdHandle) {
      ...
      b.textContent = "🔄 Reconectar resumen (1 clic)";
      b.classList.remove("vinculado");
      b.title = "Ya tienes un archivo vinculado; solo falta confirmar el permiso del navegador.";
    } else {
  ...
  // ---------- Puente con Google Drive (Apps Script) ----------
  // Un pequeño programa en la cuenta de Google del usuario recibe el resumen y lo
  // guarda en una carpeta «PRIORITY» de su Drive. Así no hace falta instalar nada
  // ni exportar a mano: cada cambio viaja solo.
  const DRIVE_KEY = "priority_drive_v1";
  let drive = { url: "", clave: "", ultimoRespaldo: 0 };
  let driveTimer = null;

  function cargarDrive() {
    try {
      const g = JSON.parse(localStorage.getItem(DRIVE_KEY));
      if (g && typeof g === "object") drive = Object.assign(drive, g);
    } catch (e) {}
    if (!drive.clave) drive.clave = uid().replace(/-/g, "").slice(0, 24);
    actualizarBotonDrive();
  }
  function guardarDrive() {
    try { localStorage.setItem(DRIVE_KEY, JSON.stringify(drive)); } catch (e) {}
    actualizarBotonDrive();
  }
  function actualizarBotonDrive() {
    ...
    if (!b) return;
    if (drive.url) {
      b.textContent = "☁ Drive conectado ✓";
      b.classList.add("vinculado");
      b.title = "Cada cambio se guarda solo en la carpeta PRIORITY de tu Drive. Clic para revisar o cambiar la conexión.";
    } else {
      b.textContent = "☁ Conectar con Google Drive…";
      b.classList.remove("vinculado");
      b.title = "Guarda el resumen en tu Drive automáticamente, sin instalar nada";
    }
  }

  // Código del Apps Script, ya con la clave del usuario puesta
  function codigoAppsScript() {
    return [
      "/** PRIORITY · Puente con Google Drive",
      " *  Guarda el resumen de tus expedientes en la carpeta CARPETA de tu Drive.",
      " *  No toca ningún otro archivo. */",
      "",
      "const CLAVE     = '" + drive.clave + "';   // debe coincidir con la de la app",
      "const CARPETA   = 'PRIORITY';              // carpeta donde se guarda todo",
      "const PERMITIDOS = ['PRIORITY_estado.md', 'PRIORITY_copia.json']; // únicos archivos que acepta",
      "",
      "function doPost(e) {",
      "  try {",
      "    // Acepta el envío directo y también el de formulario (campo 'datos'),",
      "    // que es el que funciona cuando la app se abre con doble clic.",
      "    const crudo = (e.parameter && e.parameter.datos) ? e.parameter.datos : e.postData.contents;",
      "    const d = JSON.parse(crudo);",
      "    if (!d || d.clave !== CLAVE) return respuesta_({ ok: false, error: 'Clave incorrecta' });",
      "    const carpeta = carpetaDestino_();",
      "    const nombre  = String(d.nombre || 'PRIORITY_estado.md');",
      "    if (PERMITIDOS.indexOf(nombre) < 0) return respuesta_({ ok: false, error: 'Archivo no permitido' });",
      "    const it = carpeta.getFilesByName(nombre);",
      "    if (it.hasNext()) it.next().setContent(d.contenido || '');",
      "    else carpeta.createFile(nombre, d.contenido || '', tipoMime_(nombre));",
      "    return respuesta_({ ok: true, archivo: nombre, carpeta: carpeta.getName() });",
      "  } catch (err) {",
      "    return respuesta_({ ok: false, error: String(err) });",
      "  }",
      "}",
      "",
      "function doGet() { return respuesta_({ ok: true, mensaje: 'Puente de PRIORITY activo' }); }",
      "",
      ...
      "}",
      "function tipoMime_(n) {",
      "  return n.slice(-5).toLowerCase() === '.json' ? 'application/json' : 'text/plain';",
      "}",
  ...
  async function enviarADrive(nombre, contenido) {
    if (!drive.url) return { ok: false, error: "sin conectar" };
    const cuerpo = JSON.stringify({ clave: drive.clave, nombre: nombre, contenido: contenido });
    const opciones = {
      method: "POST",
      // texto plano: evita la comprobación previa del navegador (CORS)
      headers: { "Content-Type": "text/plain;charset=utf-8" },
      body: cuerpo,
      redirect: "follow",
    };
    try {
      const r = await fetch(drive.url, opciones);
      const txt = await r.text();
      try { return JSON.parse(txt); } catch (e) {}
      // Si devuelve la página de inicio de sesión, el acceso no es «Cualquier usuario»
      if (/accounts\.google\.com|iniciar sesión|Sign in/i.test(txt)) {
        return { ok: false, motivo: "acceso", error: "El script pide iniciar sesión: en la implementación, «Quién tiene acceso» debe ser «Cualquier usuario»." };
      }
      return { ok: r.ok, motivo: r.ok ? "" : "respuesta", error: "El script respondió " + r.status };
    } catch (e) {
      // Abrir la app con doble clic (file://) hace que el navegador bloquee este
      // tipo de envío. Los formularios sí están permitidos: se manda por ahí.
      try {
        await enviarPorFormulario(cuerpo);
        return { ok: true, aciegas: true };
      } catch (e2) {
        return { ok: false, motivo: "red",
          error: "No se pudo contactar con el script. Revisa que la dirección sea la de la implementación (termina en /exec), que tengas internet y que el acceso sea «Cualquier usuario»." };
      }
    }
  }

  // Envío por formulario dentro de un marco oculto: los formularios no están
  // sujetos a las restricciones que bloquean el envío normal desde file://.
  function enviarPorFormulario(cuerpo) {
    return new Promise((res) => {
      const nombre = "priority_drive_" + Date.now();
      const marco = document.createElement("iframe");
      marco.name = nombre; marco.style.display = "none";
      document.body.appendChild(marco);
      const form = document.createElement("form");
      form.action = drive.url; form.method = "POST"; form.target = nombre;
      form.style.display = "none";
      const campo = document.createElement("input");
      campo.type = "hidden"; campo.name = "datos"; campo.value = cuerpo;
      form.appendChild(campo);
      document.body.appendChild(form);
      form.submit();
      // Se limpia después; no se puede leer la respuesta (otro dominio)
      setTimeout(() => { form.remove(); marco.remove(); res(); }, 4000);
    });
  }

  // Se llama tras cada guardado; espera un momento para no enviar en cada tecla
  function sincronizarDrive() {
    if (!drive.url) return;
    clearTimeout(driveTimer);
    driveTimer = setTimeout(async () => {
      await enviarADrive("PRIORITY_estado.md", resumenTexto());
      // Además, una copia de seguridad completa una vez al día
      const unDia = 86400000;
      if (Date.now() - (drive.ultimoRespaldo || 0) > unDia) {
        const datos = {};
        try {
        ...
        const paquete = { app: "PRIORITY", formato: 1, fecha: new Date().toISOString(), requerimientos: items.length, datos: datos };
        const res = await enviarADrive("PRIORITY_copia.json", JSON.stringify(paquete));
        if (res && res.ok) { drive.ultimoRespaldo = Date.now(); guardarDrive(); }
      }
    }, 2500);
  }

  function abrirConfigDrive(btn) {
    const rect = btn.getBoundingClientRect();
    const back = document.createElement("div"); back.className = "popover-backdrop";
    const pop = document.createElement("div"); pop.className = "cell-popover drive-pop";
    pop.innerHTML =
      '<div class="cp-label">Guardar en Google Drive sin instalar nada</div>' +
      '<ol class="drive-pasos">' +
        '<li>Entra a <b>script.google.com</b> → <b>Nuevo proyecto</b>.</li>' +
        '<li>Borra lo que haya y pega el código: <button type="button" class="btn btn-ghost btn-sm" id="drive-copiar">📋 Copiar código</button></li>' +
        '<li>Pulsa <b>Implementar → Nueva implementación</b>, tipo <b>Aplicación web</b>, ' +
            'en «Quién tiene acceso» elige <b>Cualquier usuario</b> y autoriza.</li>' +
        '<li>Copia la dirección que termina en <b>/exec</b> y pégala aquí:</li>' +
      '</ol>' +
      '<input type="text" id="drive-url" class="select drive-url" placeholder="https://script.google.com/macros/s/…/exec" value="' + esc(drive.url) + '" />' +
      '<div class="drive-foot">' +
        '<button type="button" class="btn btn-primary btn-sm" id="drive-probar">Conectar y probar</button>' +
        (drive.url ? '<button type="button" class="btn btn-ghost btn-sm" id="drive-quitar">Desconectar</button>' : "") +
        '<span class="drive-estado" id="drive-estado"></span>' +
      "</div>";
    document.body.appendChild(back); document.body.appendChild(pop);
    const w = Math.min(430, window.innerWidth - 16);
    pop.style.width = w + "px";
    pop.style.left = Math.max(8, Math.min(rect.left - w + 200, window.innerWidth - w - 8)) + "px";
    pop.style.top = Math.max(8, Math.min(rect.bottom + 6, window.innerHeight - pop.offsetHeight - 12)) + "px";
    const cerrar = () => { back.remove(); pop.remove(); document.removeEventListener("keydown", esche, true); };
    const esche = (e) => { if (e.key === "Escape") { e.stopPropagation(); cerrar(); } };
    back.addEventListener("mousedown", cerrar);
    document.addEventListener("keydown", esche, true);

    pop.querySelector("#drive-copiar").addEventListener("click", async (e) => {
      const codigo = codigoAppsScript();
      try { await navigator.clipboard.writeText(codigo); e.target.textContent = "✓ Copiado"; }
      catch (err) {
        const ta = document.createElement("textarea");
        ta.value = codigo; document.body.appendChild(ta); ta.select();
        try { document.execCommand("copy"); e.target.textContent = "✓ Copiado"; } catch (e2) {}
        ta.remove();
      }
    });
    pop.querySelector("#drive-probar").addEventListener("click", async () => {
      const url = pop.querySelector("#drive-url").value.trim();
      const est = pop.querySelector("#drive-estado");
      if (!/^https:\/\/script\.google\.com\/.*\/exec$/.test(url)) {
        est.textContent = "La dirección debe terminar en /exec"; est.className = "drive-estado mal"; return;
      }
      est.textContent = "Probando…"; est.className = "drive-estado";
      const previa = drive.url; drive.url = url;
      const res = await enviarADrive("PRIORITY_estado.md", resumenTexto());
      if (res && res.ok && !res.aciegas) {
        guardarDrive();
        est.textContent = "✓ Conectado. Guardado en la carpeta «PRIORITY» de tu Drive.";
        est.className = "drive-estado bien";
        toast("Drive conectado: tus expedientes se guardarán solos en la carpeta «PRIORITY».");
        setTimeout(cerrar, 1600);
      } else if (res && res.aciegas) {
        // Se envió, pero el navegador no deja leer la confirmación (típico con file://)
        guardarDrive();
        est.innerHTML = "Enviado ✓ — pero el navegador no deja confirmar la respuesta.<br>" +
          "<b>Abre tu Google Drive y busca la carpeta «PRIORITY»</b>: si el archivo está ahí, quedó bien conectado.";
        est.className = "drive-estado bien";
      } else {
        drive.url = previa;
        est.textContent = (res && res.error) ? res.error
          : "No respondió. Revisa que el acceso sea «Cualquier usuario» y que la clave del código coincida.";
        est.className = "drive-estado mal";
      }
    });
    const q = pop.querySelector("#drive-quitar");
    if (q) q.addEventListener("click", () => {
      drive.url = ""; guardarDrive(); cerrar();
      toast("Drive desconectado. Tus datos siguen intactos en esta PC.");
    });
  }
  ...
  function conectarEventos() {
    ...
    $("#btn-drive").addEventListener("click", (e) => abrirConfigDrive(e.currentTarget));
