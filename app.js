...
(function () {
  ...
  const CAMPOS_CSV = [
    ...
    "especialista", "fechaPase", "estado", "prioridad", "observaciones", "tengoExp",
  ];
  ...
  const filtro = { q: "", estado: "", prioridad: "", tipo: "", tengo: "" };
  ...
  function renderKpis() {
    ...
    const alta = items.filter((it) => it.prioridad === top).length;
    const sinExp = items.filter((it) => (it.tengoExp || "") === "NO").length;
    const pend = items.filter((it) => !it.estado || it.estado === defEstado()).length;
    $("#kpi-total").textContent = items.length;
    $("#kpi-alta").textContent = alta;
    $("#kpi-alta-label").textContent = "Prioridad " + (top || "—").toLowerCase();
    $("#kpi-sinexp").textContent = sinExp;
  ...
  function renderTabla() {
    ...
    if (!lista.length) {
      if (items.length === 0) { tbody.innerHTML = ""; vacio.classList.remove("hidden"); }
        ...
        tbody.innerHTML = `<tr><td colspan="13" class="empty" style="padding:34px">Ningún requerimiento coincide con los filtros.</td></tr>`;
      }
  ...
  function filaHTML(it) {
    ...
    let antig = "";
    if (dias !== null) antig = `<span class="antig-tag ${vieja ? "alerta" : ""}">${dias} d en UPROG</span>`;
    const tengo = it.tengoExp || "";
    const tengoBadge = tengo
      ? `<span class="badge ${tengo === "SÍ" ? "tengo-si" : "tengo-no"}">${esc(tengo)}</span>`
      : `<span class="muted-cell">—</span>`;

    return `
      <tr class="${vieja ? "row-vieja" : ""}">
        <td class="num cell-strong">${esc(it.nro)}</td>
        <td>${badge(it.prioridad, COL.prioridad)}</td>
        <td>${badge(it.estado, COL.estado)}</td>
        <td>${fmtFecha(it.fechaIngreso)}${antig}</td>
        <td class="cell-strong">${esc(it.expLogistica) || "—"}</td>
        <td>${esc(it.expDireccion) || "—"}</td>
        <td class="cell-doc">${esc(it.docArea) || "—"}</td>
        <td class="cell-area">${esc(it.areaUsuaria) || "—"}</td>
        <td>${badge(it.tipo, COL.tipo)}</td>
        <td class="cell-item">
          <span class="item-main">${esc(it.item) || esc(it.denominacion) || "—"}</span>
          ${it.denominacion ? `<span class="item-sub" title="${esc(it.denominacion)}">${esc(it.denominacion)}</span>` : ""}
        </td>
        <td>${esc(it.especialista) || "<span class='muted-cell'>—</span>"}</td>
        <td>${tengoBadge}</td>
        <td>
  ...
  function badge(valor, mapa) {
    if (!valor) return `<span class="muted-cell">—</span>`;
  ...
  function guardarConfigDesdeEditor() {
    ...
    // Aplicar renombrados y eliminaciones a los requerimientos
  ...
  function importarCSV(texto) {
    ...
    for (let i = 1; i < filas.length; i++) {
      ...
      let tengo = get("tengoExp").toUpperCase();
      tengo = tengo.startsWith("S") ? "SÍ" : (tengo.startsWith("N") ? "NO" : "");
      ...
      items.push({
        ...
        observaciones: get("observaciones"), tengoExp: tengo, creado: Date.now(),
      });
      n++;
    }
  ...
  function cargarJunio() {
    ...
    DATOS_JUNIO.forEach((d) => {
      ...
      items.push(Object.assign({ id: uid(), creado: Date.now() }, d));
      n++;
    });
    guardar(); render();
  ...
  function resetFiltros() {
    filtro.q = ""; filtro.estado = ""; filtro.prioridad = ""; filtro.tipo = ""; filtro.tengo = "";
    $("#buscar").value = ""; $("#filtro-estado").value = ""; $("#filtro-prioridad").value = "";
    $("#filtro-tipo").value = ""; $("#filtro-tengo").value = "";
  }
